...
static int ctp_fetch_sysconfig_para(void)
{
        ...
        if(SCRIPT_PARSER_OK != script_parser_fetch("ctp_para", "ctp6_used", &ctp_used, 1)){
        ...
        if(SCRIPT_PARSER_OK != script_parser_fetch_ex("ctp_para", "ctp6_name", (int *)(&name), &type, sizeof(name)/sizeof(int))){
                pr_err("%s: script_parser_fetch err. \n", __func__);
                goto script_parser_fetch_err;
        }
        ...
        if(SCRIPT_PARSER_OK != script_parser_fetch("ctp_para", "ctp6_twi_addr", &twi_addr, sizeof(twi_addr)/sizeof(__u32))){
                pr_err("%s: script_parser_fetch err. \n", name);
                goto script_parser_fetch_err;
        }
...
#else
        printk("config GT8105 2+1, by hawk.it@tiscali.it\n");
        const uint8_t config_info[] = {
         
		0x65, 0x03, 0x04, 0x00, 0x03, 0x00, 0x0a, 0x21, 0x1e, 0xe7, 0x32, 0x02, 0x05, 0x10, 0x4c, 0x4f,
		0x4f, 0x20, 0x07, 0x00, 0x80, 0x80, 0x3c, 0x5a, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09, 0x08, 0x07,
...
COORDINATE_POLL:
        ...
        if(touch_num)
        {
                for(index=0; index<touch_num; index++)
                {
                        ...
                        /*printk("before swap: input_x = %d, input_y = %d. \n",input_x, input_y);
                        if(1 == exchange_x_y_flag){
                                swap(input_x, input_y);
                        }

                        printk("after swap:  input_x = %d, input_y = %d. \n",input_x, input_y);
			*/
                        if((input_x > ts->abs_x_max)||(input_y > ts->abs_y_max))continue;
                        //printk("accepted:    input_x = %d, input_y = %d. \n",input_x, input_y);
